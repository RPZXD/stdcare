<?php
/**
 * Sub-View: SDQ Report by Room (Officer)
    ...
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 mb-10 no-print">
        <div>
            <h2 class="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-3">
                ...
                รายงานผล <span class="text-rose-600 italic">SDQ</span> รายห้อง
            </h2>
            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1 italic pl-13">Strengths & Difficulties Questionnaire • By Room</p>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function loadSDQTable() {
        ...
        if (classVal && roomVal) {
            ...
            fetch('api/ajax_sdq_room_table.php?class=' + encodeURIComponent(classVal) + '&room=' + encodeURIComponent(roomVal))
                .then(res => res.text())
                .then(html => {
    ...
    $roomSelect.on('change', loadSDQTable);
