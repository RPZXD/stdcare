<?php
/**
 * API: Screening 11 Result Summary by Class Level
...
// Total students count for the class
$totalStudents = 0;
foreach ($roomList as $r) {
    $stmt = $db->prepare("SELECT COUNT(*) FROM student WHERE Stu_major = :class AND Stu_room = :room AND Stu_status = 1");
    $stmt->execute(['class' => $class, 'room' => $r]);
...
<div class="mb-10 animate-fadeIn overflow-x-auto">
    <div class="min-w-[800px] glass-effect rounded-[2.5rem] p-8 border border-white/40 shadow-xl shadow-indigo-500/5">
        <div class="flex items-center gap-6 mb-8 border-b border-slate-100 dark:border-slate-800 pb-6">
            ...
            <div>
                <h3 class="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight">คัดกรอง 11 ด้าน: สรุปภาพรวมระดับชั้น ม.<?= $class ?></h3>
        ...
        <div class="grid grid-cols-11 gap-2">
            <?php foreach ($screenFields as $key => $label): 
                $sum = $classSummary[$key] ?? ['normal' => 0, 'risk' => 0, 'problem' => 0];
    ...
    <table class="w-full text-left border-separate border-spacing-y-2">
        ...
        <tbody class="font-bold text-slate-700 dark:text-slate-300">
            ...
            <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all">
                <td class="px-6 py-5 rounded-l-2xl bg-white dark:bg-slate-900 shadow-sm border-y border-l border-slate-100 dark:border-slate-800" data-label="ห้องเรียน">
                    <div class="flex items-center gap-4">
                        <span class="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-indigo-500 text-[13px] font-black italic"><?= $r ?></span>
                        <div class="text-[14px] font-black text-slate-800 dark:text-white">ห้อง <?= $class ?>/<?= $r ?></div>
                    </div>
                </td>
                <?php foreach ($screenFields as $key => $label): 
                    $n = $sum[$key]['normal'] ?? 0;
            ...
            <tr class="bg-indigo-600 text-white shadow-xl shadow-indigo-600/20">
                <td class="px-6 py-6 rounded-l-[1.5rem] font-black italic">ภาพรวมระดับชั้น <?= $class ?></td>
                <?php foreach ($screenFields as $key => $label): 
                    $sn = $classSummary[$key]['normal'] ?? 0;
                    $sr = $classSummary[$key]['risk'] ?? 0;
                    $sp = $classSummary[$key]['problem'] ?? 0;
                ?>
