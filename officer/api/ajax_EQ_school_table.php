<?php
/**
 * API: EQ Result Summary for Full School
...
$schoolSummary = [];
$totalAll = $totalHave = $totalVeryGood = $totalGood = $totalMid = $totalLow = 0;

foreach ($classList as $class) {
    $sum = $EQ->getEQClassRoomSummary($class, '', $pee, $term);
    $schoolSummary[$class] = $sum;
    $totalAll += $sum['total'];
    $totalHave += $sum['have'];
    $totalVeryGood += $sum['verygood'];
    $totalGood += $sum['good'];
    $totalMid += $sum['mid'];
    $totalLow += $sum['low'];
}
        ...
        <div class="relative z-10 flex flex-col lg:flex-row items-center gap-10">
            ...
            <div class="flex flex-col items-center shrink-0">
                ...
                <div class="mt-4 text-center">
                    <p class="text-[11px] font-black text-amber-500 uppercase tracking-widest italic">ทั้งโรงเรียน <?= $totalHave ?> / <?= $totalAll ?> คน</p>
                </div>
            </div>
            ...
            <div class="flex-1 w-full grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div class="bg-white/50 dark:bg-slate-900/50 p-6 rounded-[2rem] border border-white/50 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center group hover:scale-105 transition-all">
                    <span class="text-3xl font-black text-emerald-500 mb-1"><?= $totalVeryGood ?></span>
                    ...
                    <div class="w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full mt-4 overflow-hidden">
                        <div class="bg-emerald-500 h-full rounded-full transition-all duration-1000" style="width: <?= $totalHave > 0 ? ($totalVeryGood/$totalHave)*100 : 0 ?>%"></div>
                    </div>
                </div>
                <div class="bg-white/50 dark:bg-slate-900/50 p-6 rounded-[2rem] border border-white/50 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center group hover:scale-105 transition-all">
                    <span class="text-3xl font-black text-amber-500 mb-1"><?= $totalGood ?></span>
                    ...
                    <div class="w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full mt-4 overflow-hidden">
                        <div class="bg-amber-500 h-full rounded-full transition-all duration-1000" style="width: <?= $totalHave > 0 ? ($totalGood/$totalHave)*100 : 0 ?>%"></div>
                    </div>
                </div>
                <div class="bg-white/50 dark:bg-slate-900/50 p-6 rounded-[2rem] border border-white/50 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center group hover:scale-105 transition-all">
                    <span class="text-3xl font-black text-blue-500 mb-1"><?= $totalMid ?></span>
                    ...
                    <div class="w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full mt-4 overflow-hidden">
                        <div class="bg-blue-500 h-full rounded-full transition-all duration-1000" style="width: <?= $totalHave > 0 ? ($totalMid/$totalHave)*100 : 0 ?>%"></div>
                    </div>
                </div>
                <div class="bg-white/50 dark:bg-slate-900/50 p-6 rounded-[2rem] border border-white/50 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center group hover:scale-105 transition-all">
                    <span class="text-3xl font-black text-rose-500 mb-1"><?= $totalLow ?></span>
                    ...
                    <div class="w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full mt-4 overflow-hidden">
                        <div class="bg-rose-500 h-full rounded-full transition-all duration-1000" style="width: <?= $totalHave > 0 ? ($totalLow/$totalHave)*100 : 0 ?>%"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <table class="w-full text-left border-separate border-spacing-y-2">
        <thead>
            <tr class="bg-slate-50/50 dark:bg-slate-900/50">
                ...
                <th class="px-3 py-4 text-[10px] font-black text-emerald-600 uppercase tracking-widest italic text-center">ดีมาก</th>
                <th class="px-3 py-4 text-[10px] font-black text-amber-600 uppercase tracking-widest italic text-center">ดี</th>
                <th class="px-3 py-4 text-[10px] font-black text-blue-600 uppercase tracking-widest italic text-center">ปานกลาง</th>
                <th class="px-3 py-4 text-[10px] font-black text-rose-600 uppercase tracking-widest italic rounded-r-2xl text-center">ปรับปรุง</th>
            </tr>
        </thead>
        <tbody class="font-bold text-slate-700 dark:text-slate-300">
            ...
            <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all">
                <td class="px-6 py-5 rounded-l-2xl bg-white dark:bg-slate-900 shadow-sm border-y border-l border-slate-100 dark:border-slate-800" data-label="ระดับชั้น">
                    <div class="flex items-center gap-4">
                        <span class="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-amber-500 text-[13px] font-black italic">ม.<?= $class ?></span>
                ...
                <td class="px-3 py-5 bg-white dark:bg-slate-900 shadow-sm border-y border-slate-100 dark:border-slate-800 text-center text-emerald-600" data-label="ดีมาก">
                    <span class="text-sm font-black italic"><?= $sum['verygood'] ?></span>
                </td>
                <td class="px-3 py-5 bg-white dark:bg-slate-900 shadow-sm border-y border-slate-100 dark:border-slate-800 text-center text-amber-600" data-label="ดี">
                    <span class="text-sm font-black italic"><?= $sum['good'] ?></span>
                </td>
                <td class="px-3 py-5 bg-white dark:bg-slate-900 shadow-sm border-y border-slate-100 dark:border-slate-800 text-center text-blue-600" data-label="ปานกลาง">
                    <span class="text-sm font-black italic"><?= $sum['mid'] ?></span>
                </td>
                <td class="px-3 py-5 rounded-r-2xl bg-white dark:bg-slate-900 shadow-sm border-y border-r border-slate-100 dark:border-slate-800 text-center text-rose-600" data-label="ปรับปรุง">
            ...
            <tr class="bg-amber-500 shadow-xl shadow-amber-500/20 text-white animate-pulse">
                ...
                <td class="px-3 py-6 text-center font-black italic"><?= $totalVeryGood ?></td>
                <td class="px-3 py-6 text-center font-black italic"><?= $totalGood ?></td>
                <td class="px-3 py-6 text-center font-black italic"><?= $totalMid ?></td>
                <td class="px-3 py-6 rounded-r-[1.5rem] text-center font-black italic"><?= $totalLow ?></td>
            </tr>
        </tbody>
    </table>
</div>
