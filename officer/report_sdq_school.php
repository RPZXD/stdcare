...
<div class="mb-6">
    <h2 class="text-xl font-bold text-red-600 flex items-center gap-2 mb-4">
        🧠 รายงานผล SDQ (ภาพรวมทั้งโรงเรียน)
    </h2>
    <div class="mb-4 flex justify-end">
        <button onclick="printSDQSchoolTable()" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded shadow font-semibold print:hidden">
            🖨️ พิมพ์รายงาน
        </button>
    </div>
            ...
            .then(html => {
                ...
                let header = `
                    <div id="print-header" class="mb-4 text-center">
                        <div class="font-bold text-xl">รายงานผล SDQ (ภาพรวมทั้งโรงเรียน)</div>
                        <div class="text-lg">
    ...
    function printSDQSchoolTable() {
        let printContents = document.getElementById('print-area').innerHTML;
        let win = window.open('', '', 'width=900,height=700');
        win.document.write('<html><head><title>รายงานผล SDQ (โรงเรียน)</title>');
