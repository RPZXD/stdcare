<?php
/**
 * Sub-View: SDQ School-wide Report (Officer)
...
<div class="animate-fadeIn">
    ...
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 mb-10">
        <div>
            <h2 class="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-3">
                ...
                สรุปผลการประเมิน <span class="text-rose-600 italic">SDQ</span> (ภาพรวมโรงเรียน)
            </h2>
            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1 italic pl-13">School-wide Strengths & Difficulties Summary • <?= htmlspecialchars($pee) ?></p>
        </div>
        
        <div class="flex gap-2 no-print">
            <button onclick="loadSDQSchoolTable()" class="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center gap-2">
    ...
    <!-- Content Container -->
    <div id="sdq-table-container" class="space-y-10">
...
function loadSDQSchoolTable() {
    ...
    fetch('api/ajax_sdq_school_table.php')
        .then(res => res.text())
        .then(html => {
