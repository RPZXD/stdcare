<?php
/**
 * Sub-View: EQ Report by Class Level (Officer)
    ...
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 mb-10">
        <div>
            <h2 class="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-3">
                ...
                รายงานผล <span class="text-amber-600 italic">EQ</span> รายระดับชั้น
            </h2>
            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1 italic pl-13">Emotional Quotient Assessment • By Grade Level</p>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function loadEQTable() {
        ...
        if (classVal) {
            ...
            fetch('api/ajax_EQ_class_table.php?class=' + encodeURIComponent(classVal))
                .then(res => res.text())
                .then(html => {
    ...
    $classSelect.on('change', loadEQTable);
    loadEQTable();
});
</script>
