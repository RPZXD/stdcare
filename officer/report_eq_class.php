...
<div class="mb-6">
    ...
    <div class="mb-4 flex justify-end">
        <button onclick="printEQClassTable()" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded shadow font-semibold print:hidden">
            🖨️ พิมพ์รายงาน
        </button>
    </div>
    ...
    function loadEQTable() {
        ...
        if (classValue) {
            ...
            fetch('api/ajax_EQ_class_table.php?class=' + encodeURIComponent(classValue))
                .then(response => response.text())
                .then(html => {
                    // เพิ่มหัวกระดาษตอน print
                    let header = `
                        <div id="print-header" class="mb-4 text-center">
                            <div class="font-bold text-xl">รายงานผล EQ (รายชั้น)</div>
                            <div class="text-lg">
            ...
            document.getElementById('EQ-table-container').innerHTML = '<div class="text-center text-gray-400 py-6">กรุณาเลือกชั้น</div>';
        }
    }
    ...
    function printEQClassTable() {
        let printContents = document.getElementById('print-area').innerHTML;
        let win = window.open('', '', 'width=900,height=700');
        win.document.write('<html><head><title>รายงานผล EQ (รายชั้น)</title>');
