...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Load Classes
    fetch('../teacher/api/api_get_classes.php')
        .then(res => res.json())
        .then(data => {
    ...
    if(classSelect) {
        classSelect.addEventListener('change', function() {
            ...
            if (classVal) {
                fetch(`../teacher/api/api_get_rooms.php?class=${classVal}`)
                    .then(res => res.json())
                    .then(data => {
    ...
    if(filterForm) {
        filterForm.addEventListener('submit', function(e) {
            ...
            Swal.showLoading();
            fetch(`../teacher/api/ajax_get_class_visit_home.php?class=${classVal}&room=${roomVal}`)
                .then(res => res.json())
                .then(data => {
                    Swal.close();
                    if(data.success) {
    ...
    document.getElementById('exportExcelBtn').addEventListener('click', () => {
        ...
        const roomVal = document.getElementById('roomSelect').value;
        if(classVal && roomVal) {
            window.open(`../teacher/api/export_visit_home.php?class=${classVal}&room=${roomVal}`, '_blank');
        }
    });
});
...
function viewVisit(id) {
    ...
    fetch(`../teacher/api/get_visit_details_full.php?student_id=${id}&class=${classVal}&room=${roomVal}`)
        .then(res => res.text())
        .then(html => {
