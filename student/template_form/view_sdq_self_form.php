...
<div class="bg-blue-100 text-blue-800 px-4 py-3 rounded-md mb-4">
    📋 <strong>คำชี้แจง:</strong> แสดงคำตอบที่เลือกไว้ในช่วง 6 เดือนที่ผ่านมา
</div>
<table class="w-full border-collapse border border-gray-300 mb-4">
    <thead>
        <tr class="bg-blue-500 text-white text-center">
            ...
            <th class="border border-gray-300 px-4 py-2">หมวดหมู่</th>
    ...
    <tbody>
        <?php foreach ($questions as $index => [$id, $text, $category]): ?>
            <tr class="hover:bg-gray-50">
                ...
                <td class="border border-gray-300 px-4 py-2"><?= htmlspecialchars($text) ?></td>
                <td class="border border-gray-300 px-4 py-2 text-center"><?= $category ?></td>
                <td class="border border-gray-300 px-4 py-2 text-center font-semibold">
                    <?php
                        $val = $answers[$id] ?? '';
                        echo $val !== '' && isset($choices[$val]) ? $choices[$val] : '-';
                    ?>
                </td>
            </tr>
        <?php endforeach; ?>
