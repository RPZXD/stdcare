...
// ฟิลด์ที่เป็น array ต้องแปลงเป็น json string
$arrayFields = [
...
if (isset($data['special_ability']) && $data['special_ability'] === 'มี') {
    ...
    // ถ้ามี special_ability_detail จาก hidden input ให้ใช้ค่านั้น (กรณี JS สร้าง JSON ไว้แล้ว)
    if (isset($data['special_ability_detail']) && !empty($data['special_ability_detail'])) {
        $data['special_ability_detail'] = $data['special_ability_detail'];
    } else {
