                    ...
                    <div class="card-body text-center">
                            <img src="../dist/img/logo-phicha.png" alt="Phichai Logo" class="mx-auto w-16 h-16 mb-3">
                                <h5 class="text-lg font-bold">
                                    🏠 การคัดกรองนักเรียนรายบุคคล 11 ด้าน <br>
                                    ระดับชั้นมัธยมศึกษาปีที่ <?= $class."/".$room; ?>
                                </h5>
                        
                        <div class="text-left mt-4">
                            <button type="button" class="bg-rose-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-rose-600 mb-3" onclick="window.location.href='report_screen_all.php'">
                            📊 รายงานสถิติข้อมูลการคัดกรองนักเรียนรายบุคคล 11 ด้าน 📊
                            </button>
                                    ...
                                    <table id="record_table" class="display table-bordered table-hover" style="width:100%">
                                        <thead class="thead-secondary bg-indigo-500 text-white">
                                            <tr >
                                                ...
                                                <th class="text-center">คัดกรอง</th>
                                                <th class="text-center">แปลผล</th>
                                                <th class="text-center">สถานะการคัดกรอง</th>
                                            </tr>
                                        </thead>
                                        <tbody>
...
async function loadTable() {
    try {
        ...
        const response = await $.ajax({
            url: 'api/fetch_screen_classroom.php',
            method: 'GET',
            dataType: 'json',
        ...
        function createResultButton(isHave, item) {
            if (!isHave) {
                return `<span class="text-danger">❌ ยังไม่ได้คัดกรอง</span>`;
            }
        ...
        if (response.data.length === 0) {
            ...
            response.data.forEach((item, index) => {
                table.row.add([
                    ...
                    item.Stu_id,
                    item.full_name,
                    createActionButton(item.screen_ishave === 1, item),
                    createResultButton(item.screen_ishave === 1, item),
                    item.screen_ishave === 1 ? '✅ คัดกรองแล้ว' : '❌ ยังไม่ได้คัดกรอง',
                ]);
            });
        }
...
window.openAddModal = function(studentId, studentName, studentNo, studentClass, studentRoom, Term, Pee) {
    $.ajax({
        url: 'template_form/form_screen.php',
        method: 'GET',
        ...
        success: function(response) {
                    ...
                    <div class="modal-dialog modal-lg" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="screenModalLabel">แบบฟอร์มการคัดกรองนักเรียนรายบุคคล 11 ด้าน</h5>
            ...
            $('#saveScreen').on('click', function() {
                const formData = $('#screenForm').serialize();
...
window.openEditModal = function(studentId, studentName, studentNo, studentClass, studentRoom, Term, Pee) {
    $.ajax({
        url: 'template_form/form_screen_edit.php',
        method: 'GET',
        data: { student_id: studentId, student_name: studentName, student_no: studentNo, student_class: studentClass, student_room: studentRoom, pee: Pee, term: Term },
        success: function(response) {
            // Create and display the modal
            const modalHtml = `
                <div class="modal fade" id="editscreenModal" tabindex="-1" role="dialog" aria-labelledby="editscreenModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-xl" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="editscreenModalLabel">แบบฟอร์มแก้ไขข้อมูลการคัดกรองนักเรียนรายบุคคล 11 ด้าน</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
            ...
            $('#updateSDQ').on('click', function() {
                const formData = $('#screenEditForm').serialize(); // Assuming the form has id="sdqEditForm"
            ...
            // Remove modal from DOM after hiding
            $('#editscreenModal').on('hidden.bs.modal', function() {
                $(this).remove();
            });
        },
        error: function() {
...
window.openResultModal = function(studentId, studentName, studentNo, studentClass, studentRoom, Term, Pee) {
    $.ajax({
        url: 'template_form/form_screen_result.php',
        method: 'GET',
        data: { student_id: studentId, student_name: studentName, student_no: studentNo, student_class: studentClass, student_room: studentRoom, pee: Pee, term: Term },
        success: function(response) {
            // Create and display the modal
                ...
                <div class="modal fade" id="resultModal" tabindex="-1" role="dialog" aria-labelledby="resultModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg" role="document">
                        <div class="modal-content" id="resultModalContent">
                            <div class="modal-header">
                                <h5 class="modal-title" id="resultModalLabel">แปลผลข้อมูลการคัดกรองนักเรียนรายบุคคล 11 ด้าน</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
